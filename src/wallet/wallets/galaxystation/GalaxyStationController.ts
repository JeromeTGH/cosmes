...
export class GalaxyStationController extends WalletController {
  ...
  protected async connectWalletConnect<T extends string>(
    ...
    for (const { chainId } of chains) {
      // GalaxyStation mobile's WallectConnect only supports these chains
      // TODO: update when GalaxyStation mobile supports more chains
    ...
    // GalaxyStation mobile only returns 1 address for now
    // TODO: update when GalaxyStation mobile supports more chains
    const address = wc.accounts[0];
    for (let i = 0; i < chains.length; i++) {
      const { chainId, rpc, gasPrice } = chains[i];
      try {
        // Since GalaxyStation's WalletConnect doesn't support getting pub keys, we
        ...
        wallets.set(
          chainId,
          new GalaxyStationWalletConnectV1(
            wc,
            chainId,
            key,
            address,
  ...
  protected async connectExtension<T extends string>(chains: ChainInfo<T>[]) {
    ...
    // This method never throws on GalaxyStation
  ...
  protected registerAccountChangeHandlers() {
    ...
    // GalaxyStation's WalletConnect v1 doesn't support account change events
  }
